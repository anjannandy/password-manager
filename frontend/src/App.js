...
const Sidebar = ({ user, onLogout, sidebarWidth, onResizeStart }) => {
    ...
    return (
        ...
        >
            <div className="flex flex-col h-full flex-1 bg-black text-white shadow-2xl overflow-hidden">
                {/* Header */}
                <div className="p-4 border-b border-gray-800 flex items-center gap-3 h-20 shrink-0">
                    ...
                    {!isCollapsed && (
                        <div className="flex flex-col overflow-hidden whitespace-nowrap">
                            ...
                            <span className="text-[10px] uppercase tracking-widest text-gray-400 font-semibold">Vault Manager v1.5</span>
                        </div>
                    )}
                </div>
                ...
                <nav className="flex-1 mt-6 px-2 space-y-1 overflow-y-auto overflow-x-hidden">
                    {navItems.map((item) => (
                        ...
                        >
                            <div className={`shrink-0 ${isActive(item.path) ? 'text-black' : 'text-gray-500 group-hover:text-gray-200'}`}>
                                {item.icon}
                            </div>
                ...
                {/* User & Logout */}
                <div className="p-3 border-t border-gray-800 bg-black/50 shrink-0">
                    <div className={`flex items-center gap-3 p-2 mb-3 rounded-xl ${!isCollapsed ? 'bg-gray-900' : ''}`}>
                        <div className="w-10 h-10 rounded-xl bg-white text-black flex items-center justify-center font-bold shadow-lg shrink-0">
                    ...
                    <button
                        onClick={onLogout}
                        className={`w-full flex items-center gap-3 p-3 text-gray-400 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all duration-200 group ${isCollapsed ? 'justify-center' : ''}`}
                        title="Logout"
                    >
            ...
            <div
                ...
                className="w-1.5 cursor-col-resize flex items-center justify-center bg-black hover:bg-gray-700 active:bg-gray-600 transition-colors group shrink-0"
                title="Drag to resize"
            >
                <GripVerticalIcon size={12} className="text-gray-600 group-hover:text-gray-300" />
            </div>
        </div>
    );
};
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {loading ? (
                            ...
                            <tr key={h.id} className="hover:bg-gray-50 transition-colors">
                                <td className="px-6 py-4 font-mono text-sm">{h.ipAddress}</td>
...
const App = () => {
    ...
    return (
        <Router>
            <div className="flex h-screen w-screen bg-white text-black font-sans overflow-hidden">
                {user && (
                    <Sidebar 
                        user={user} 
                        onLogout={onLogout} 
                ...
                <div className="flex-1 min-w-0 h-screen overflow-auto bg-white">
                    <Routes>
